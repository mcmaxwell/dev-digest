...
export function helpText(): string {
  ...
  return [
    ...
    '',
    'USAGE',
    '  devdigest review [--mode <mode>] [--base <ref>] [--agent <slug|id>]',
    '                   [--fail-on <severity>] [--severity-min <severity>]',
    '                   [--format text|json] [--api <url>]',
    '  devdigest review --mode branch --base <ref> --repo <owner/name> --pr <n>',
    '                   [--post-as <how>] [--ci-result <path>]      # inside CI',
    '',
    'MODES',
    ...modes,
    ...
    '  --base <ref>           The ref --mode branch diffs against, e.g. main. The diff is',
    '                         taken from the MERGE BASE, so it matches the pull request.',
    ...
    '',
    'RUNNING INSIDE CI',
    '  --repo <owner/name>    With --pr, the review is recorded as a CI run and DevDigest',
    '  --pr <number>          posts it to that pull request. The agent must already be',
    '                         exported to that repository, or the call is refused.',
    '  --post-as <how>        github_review | pr_comment | none. Only a github_review can',
    '                         request changes, and only when the gate fires.',
    '  --ci-result <path>     Write the run summary (counts, cost, duration) as JSON, for',
    '                         upload-artifact. Never contains finding text.',
    '  This CLI holds NO GitHub token and no model key: it sends the diff, and the',
    '  DevDigest server does the review and the posting.',
    '',
